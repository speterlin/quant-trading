...
def get_coinmarketcap_coin_data(coin):
    market_data = {}
...
def get_coingecko_coin_data(coin):
    data = {}
...
def get_coin_data(coin, date=None, historical=False, retry_current_if_no_historical_market_data=False):
    ...
    if not historical: # or date == datetime.now().strftime('%d-%m-%Y')
        ...
        data = get_coinmarketcap_coin_data(coin)
    else:
...
def get_coin_data_granular(coin, from_timestamp, to_timestamp, currency='btc'): # time is in local time (PST)
...
def get_coins_markets(currency='btc', per_page=250, pages=1): # if decide to use less than max 250 entries per_page need to change error_str of _fetch_data executions
...
def get_coinmarketcap_coins_markets(pages=10): # refactor add market cap
    data = {}
    market_cap_rank = 0
...
def get_coingecko_coins_markets(pages=10):
    data = {}
    for page in range(1, pages+1):
...
def save_coins_data(date, pages=10): # date is in format '%Y-%m-%d' # maybe refactor and add date if code block runs at a datetime.now() which is close to the next day, if runs longer no need to add date since market runs 24/7 # number of pages should be constant
    coins = _fetch_data(get_coinmarketcap_coins_markets, params={'pages': pages}, error_str=" - No " + "" + " coins markets data with pages: " + str(pages) + " on: " + str(datetime.now()), empty_data={}) # refactor all - ensure error_str have date: # 'currency': 'btc',
    ...
    count = 0
    for coin_id, symbol_and_market_data in coins.items(): # here and throughout where iterating over get_coins_markets assuming that all necessary keys are there (not checking for example if 'market_cap_rank', 'current_price', 'symbol' in coin) (has been the case in all cases observed)
        count += 1
        if count % 134 == 0:
...
def kucoin_usdt_check_arbitrages(pages=10):
    ...
    kucoin_pairs_with_price_and_vol_current = _fetch_data(get_kucoin_pairs, params={}, error_str=" - Kucoin get tickers error on: " + str(datetime.now()), empty_data={})
    coins = _fetch_data(get_coinmarketcap_coins_markets, params={'pages': pages}, error_str=" - No " + "" + " coins markets data with pages: " + str(pages) + " on: " + str(datetime.now()), empty_data={}) # faster than iterating through binance pairs and retrieving price for each coin, allows for more flexibility with exchanges etc.
                ...
                if (idx == len(coin_data_granular_in_btc['prices']) - 1): # some days ends earlier than 16:59 like 2020-06-08 ends 15:59 (fet), 16:12 (edo), 16:29 (stmx, bnt) issue with coingecko data (but also when when run on different occasions returns different results for same day i.e. with end_day 6/14/2020 and different start days (6/17/2020 and same start days) return different end times)
                    if stop_day == end_day:
                        ...
                        if END_DAY_OPEN_POSITIONS_GTRENDS_15D:
                            ...
                            google_trends_slope = trendline(google_trends.sort_values('date', inplace=False, ascending=True)[coin_search_term], reverse_to_ascending=True) if not google_trends.empty else float("NaN")
...
def update_portfolio_buy_and_sell_coins(portfolio, coins_to_buy, coins_to_sell, stop_day, end_day, paper_trading, back_testing, **params): # refactor and add back_testing
    ...
    if not back_testing:
        coins_data = _fetch_data(get_coinmarketcap_coins_markets, params={'pages': 10}, error_str=" - No " + "" + " coins markets data with pages: " + str(10) + " on: " + str(datetime.now()), empty_data={})
...
def get_kucoin_assets(account_type="trade", other_coins_symbol_to_id=None, pages=10):
    ...
    coins_data = _fetch_data(get_coinmarketcap_coins_markets, params={'pages': pages}, error_str=" - No " + "" + " coins markets data with pages: " + str(pages) + " on: " + str(datetime.now()), empty_data={})
    coins_symbol_to_id = {**{'btc': 'bitcoin', 'bnb': 'binancecoin', 'fet': 'fetch-ai', 'kmd': 'komodo', 'cnd': 'cindicator', 'coti': 'coti', 'tfuel': 'theta-fuel', 'tomo': 'tomochain', 'gto': 'gifto', 'btg': 'bitcoin-gold'}, **{symbol_and_market_data['symbol']: coin_id for coin_id, symbol_and_market_data in coins_data.items()}}
