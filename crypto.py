...
def get_saved_coins_data(date): # date is a string in format '%Y-%m-%d'
    try:
        ...
        # maybe refactor and add some columns like "Google Trends"
...
binance_client = BinanceClient(personal.binance_key, personal.binance_secret)
...
def binance_trade_coin_btc(symbol_pair, trade=None, side=None, btc_invest=None, quantity=None, open_time=0, paper_trading=True, other_notes=None): # don't like non-boolean value for trade value but having two bool values would complicate matters (i.e if both set True etc.) # price_in_btc=None # assuming binance processes open_order (if order not immediately filled) almost immediately (and that this open_order sometimes filled immediately since often getting ~Filled) # can use recvWindow with api
    ...
    if not order['fills']: # maybe refactor might be able to use 'executedQty'
        time.sleep(open_time) # since order not often filled immediately, but if assuming open_order created immediately, open_time if used with another call to open_orders can be used to check if the order was filled within open_time
    ...
    trade_notes = "Filled" if (order['fills'] and not open_orders) else "Not filled" if (not order['fills'] and open_orders) else "Partially filled" if (order['fills'] and open_orders) else "~Filled" # maybe refactor - if have open_time > 0 can assume that if no order['fills'] and no open_orders that order has been Filled, might need to check balances as precautionary # precautionary - could be a situation where there is an order but no order['fills'] and no open_orders (since api processing open_order fails or open_order not immediately filled but within creating open_order and open_time - most often the case) which is not a "BTrade Error" as long as open_time=~0, can be avoided with 2 calls to open_orders
...
def update_portfolio_buy_and_sell_coins(portfolio, coins_to_buy, coins_to_sell, stop_day, end_day, paper_trading, back_testing, **params): # refactor and add back_testing
    ...
    for coin, market_cap_rank_change in coins_to_buy: # coins_market_cap_rank_change_by_factor.items()
        if coin not in portfolio['open'].index and (portfolio['balance']['btc'] >= BTC_INVEST_MIN): # can add max open positions and a waiting list to reflect real trading: # assuming always enforcing BTC_INVEST_MIN # (market_cap_rank_change >= UP_MOVE) and # maybe refactor, logic here and not in run_portfolio_algorithm to keep logic simple (even though makes tickers_to_buy/sell lists longer)
            ...
            if 'market_data' in coin_data and coin_data['market_data']['market_cap']['usd']: # refactor can probably get rid of coin_data['market_data']['market_cap']['usd'] / 'usd'/'btc' in coin_data['market_data']['market_cap']/['current_price']
                ...
                if symbol_pair in binance_pairs_with_price_current: # not retrieving new prices since whole function should execute (if done over one DAYS period) quickly
                    ...
                    if back_testing: # maybe refactor and put this logic into function binance_trade_coin_btc, maybe also include binance_check_24h_vol_and_price_in_btc in back_testing purchases
                        ...
                        quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, trade="buy", btc_invest=btc_invest, open_time=0, paper_trading=paper_trading) # maybe refactor and change binance_coin_btc... to binance_btc_..., binance_coin_btc may be good to reflect importance (exchange order)
    ...
    for coin, market_cap_rank_change in coins_to_sell:
        if coin in portfolio['open'].index: # can add short logic # not accounting for if there is a market data issue (MDI when backtesting) - if sell price and roi doesn't reflect actual, can try to postpone selling by a day # (market_cap_rank_change <= DOWN_MOVE) and
            symbol_pair = portfolio['open'].loc[coin, 'symbol'].upper() + 'BTC' # maybe refactor all and change balance variable name to order_quantity
            buy_price_in_btc, balance = portfolio['open'].loc[coin, ['buy_price(btc)', 'balance']] # maybe refactor, buy_price_in_btc here so only one call for both 'buy_price(btc)', 'balance' to portfolio.loc[]
            if back_testing:
                ...
                sell_date = datetime.now() #  binance_btc_24h_vol_in_btc , float(_fetch_data(binance_client.get_ticker, params={'symbol': symbol_pair}, error_str=" - Binance get ticker error for symbol_pair: " + symbol_pair + " on date: " + str(datetime.now().date()), empty_data={})['quoteVolume']) # *binance_pairs_with_price_current['BTCUSDT'] # maybe refactor - add back binance_btc_24h_vol_in_btc if want to check for pump and dump but complicates matters when checking for buying and selling
                ...
                quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, trade="sell", quantity=balance, open_time=0, paper_trading=(True if portfolio['open'].loc[coin, 'position'] == 'long-p' else paper_trading))
...
def run_portfolio_rr(portfolio, start_day=None, end_day=None, rr_sell=True, paper_trading=True, back_testing=False): # start_day and end_day are datetime objects # short = False, possibly add short logic # can get rid of back_testing parameter and add logic like start_day.date() < (end_day - timedelta(days=DAYS)).date(), # maybe refactor and add long_position = 'long-p' if paper_trading else 'long'
    ...
    if (back_testing and not paper_trading) or (not back_testing and ((start_day.date() < (end_day - timedelta(days=DAYS)).date()) or (end_day.date() < datetime.now().date()))): # maybe refactor, if back_testing, doesn't matter if paper_trading is set to True or False, just precautionary # don't allow back running:
    ...
    while stop_day.date() <= end_day.date():
        ...
        df_coins_interval_start, df_coins_interval_stop = get_saved_coins_data(date=(stop_day - timedelta(days=DAYS)).strftime('%Y-%m-%d')).iloc[:250], get_saved_coins_data(date=stop_day.strftime('%Y-%m-%d')).iloc[:250] # maybe refactor and make option to limit amount of coins to analyze
        ...
        if not (df_coins_interval_start.empty or df_coins_interval_stop.empty):
            ...
            for coin in list(set(df_coins_interval_start.index.values) - set(df_coins_interval_stop.index.values)):
                ...
                coins_to_sell.append([coin, df_coins_interval_start.loc[coin, 'Market Cap Rank'] - (len(df_coins_interval_start) - DOWN_MOVE)]) if rr_sell else None # market_cap_rank_change unused and inaccurate but still saving # coins_market_cap_rank_change_by_factor[coin] = df_coins_interval_start.loc[coin, 'Market Cap Rank'] - (len(df_coins_interval_start) - DOWN_MOVE)
            portfolio = update_portfolio_buy_and_sell_coins(portfolio=portfolio, coins_to_buy=coins_to_buy, coins_to_sell=coins_to_sell, stop_day=stop_day, end_day=end_day, paper_trading=paper_trading, back_testing=back_testing, binance_pairs_with_price_current=binance_pairs_with_price_current)
...
def get_binance_assets(other_coins_symbol_to_id=None, pages=1): # maybe refactor, don't pass in portfolio since assets should have current open positions in portfolio and new added positions should be in Binance top 250, but possible if run function without assets previously loaded and a coin in assets on Binance is no longer in Binance top 250 (algorithm should sell at 17PST if coin falls out of top 250 but still possible)
    ...
    coins_symbol_to_id = {**{'btc': 'bitcoin', 'bnb': 'binancecoin', 'fet': 'fetch-ai', 'kmd': 'komodo', 'cnd': 'cindicator', 'coti': 'coti', 'tfuel': 'theta-fuel', 'tomo': 'tomochain', 'gto': 'gifto', 'btg': 'bitcoin-gold'}, **{coin['symbol']: coin['id'] for coin in _fetch_data(get_coins_markets, params={'currency': 'usd', 'per_page': 250, 'pages': pages}, error_str=" - No " + "" + " coins markets data with pages: " + str(pages) + " on: " + str(datetime.now()), empty_data=[])}}
...
def retry_binance_open_orders_in_portfolio(portfolio, binance_open_orders):
    binance_pairs_with_price_current = {price['symbol']: float(price['price']) if price['price'] else 0 for price in _fetch_data(binance_client.get_all_tickers, params={}, error_str=" - Binance get all tickers error on: " + str(datetime.now()), empty_data=[])}
    for open_order in binance_open_orders:
        ...
        if not df_matching_open_options.empty: # maybe refactor and add precautionary check - shouldn't have to check len() == 1 since symbol, like coin (index) should be unique in portfolio['open'] unless two different coins have the same symbol
            ...
            buy_price_in_btc = portfolio['open'].loc[coin, 'buy_price(btc)']
            if abs((binance_pairs_with_price_current[symbol_pair] - buy_price_in_btc) / buy_price_in_btc) <= OPEN_ORDER_PRICE_DIFFERENCE: # *binance_pairs_with_price_current['BTCUSDT'] # refactor can make that a parameter also not sure if necessary, can be sl/2
                ...
                if resp['status'] == 'CANCELED': # cancelled is more British English, canceled is more American English, Binance uses canceled, grammarly.com/blog/canceled-vs-cancelled
                    original_price_in_btc = buy_price_in_btc
                    ...
                    quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, side=side, quantity=balance, open_time=0, paper_trading=paper_trading, other_notes="Retrying open order for coin " + coin + " bought on " + str(order_time))
        ...
        if not df_matching_sold_options.empty: # maybe refactor and add precautionary check - shouldn't have to check len() == 1 since symbol, trade_notes and order_time time frame (+/- 10 minutes)
            ...
            coin, sell_price_in_btc, buy_price_in_btc = portfolio['sold'].loc[idx, ['coin', 'sell_price(btc)', 'buy_price(btc)']]
            if abs((binance_pairs_with_price_current[symbol_pair] - sell_price_in_btc) / sell_price_in_btc) <= OPEN_ORDER_PRICE_DIFFERENCE: # *binance_pairs_with_price_current['BTCUSDT'] # refactor can make that a parameter also not sure if necessary, can be sl/2
                resp = _fetch_data(binance_client.cancel_order, params={'symbol': symbol_pair, 'orderId': order_id}, error_str=" - Binance cancel order error for symbol_pair: " + symbol_pair + " and orderId: " + str(order_id) + " on: " + str(datetime.now()), empty_data=[]) # maybe refactor unnecessary if order['fills']
                if resp['status'] == 'CANCELED': # cancelled is more British English, canceled is more American English, Binance uses canceled
                    original_price_in_btc = sell_price_in_btc
                    balance = original_quantity - executed_quantity
                    quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, side=side, quantity=balance, open_time=0, paper_trading=paper_trading, other_notes="Retrying open order for coin " + coin + " sold on " + str(order_time))
                    new_price_in_btc = (price_in_btc*quantity + original_price_in_btc*executed_quantity) / original_quantity
...
def portfolio_trading(portfolio, paper_trading=True, open_order_price_difference=0.15): # refactor to mimick run_portfolio_rr # short = False, possibly add short logic
    ...
    while True:
        print("<< " + str(datetime.now()) + ", paper_trading: " + str(paper_trading) + " >>")
        ...
        if (datetime.utcnow().hour == 0) and (datetime.utcnow().minute < 4): # since runs every 4 minutes
            ...
            portfolio = run_portfolio_rr(portfolio=portfolio, start_day=(datetime.now() - timedelta(days=DAYS)), end_day=datetime.now(), paper_trading=paper_trading)
        ...
        for coin in portfolio['open'].index:
            ...
            if 'market_data' in coin_data and coin_data['market_data']['market_cap']['usd']:
                ...
                print("Error retreiving market data (specifically market cap) for coin: " + coin + " on: " + str(datetime.now()))
            if price_in_btc: # maybe refactor check for if 'position' == 'long'/'long-p' in case implement shorting, price to ensure price was calculated on coingecko
                ...
                if tsl_armed:
                    ...
                    if tsl_price_in_btc_change <= TRAILING_STOP_LOSS_PERCENTAGE: # should check if price on coingecko is equal/close to price in binance
                        ...
                        quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, trade="sell", quantity=balance, open_time=0, paper_trading=(True if portfolio['open'].loc[coin, 'position'] == 'long-p' else paper_trading), other_notes=other_notes + " at ~roi " + str(price_in_btc_change)) # ~ roi since roi calculated with CoinGecko price but real roi is with Binance price
                    ...
                    quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, trade="sell", quantity=balance, open_time=0, paper_trading=(True if portfolio['open'].loc[coin, 'position'] == 'long-p' else paper_trading), other_notes=other_notes + " at ~roi " + str(price_in_btc_change))
        ...
        if datetime.now().minute >= 30 and datetime.now().minute < 34: # runs once per hour at the end of the hour (since if save data or run algorithm at beginning of hour may have conflict since saving data and running algorithm takes time)
            print(str(portfolio['sold'].tail(40).drop(['symbol', 'binance_btc_24h_vol(btc)', 'rank_rise_d', 'tsl_max_price(btc)', 'gtrends_15d'], axis=1)) + "\nSold ROI (BTC) (Real): " + str(portfolio_calculate_roi(portfolio, open_positions=False, sold_positions=True, avoid_paper_trades=True)) + "\nSold ROI (BTC) (All): " + str(portfolio_calculate_roi(portfolio, open_positions=False, sold_positions=True)) + "\nPortfolio ROI (BTC) (Real): " + str(portfolio_calculate_roi(portfolio, open_positions=True, sold_positions=True, avoid_paper_trades=True)) + "\nPortfolio Available BTC Balance: " + str(portfolio['balance']['btc']) + "\n")
            assets = get_binance_assets(other_coins_symbol_to_id=dict(zip(list(portfolio['open']['symbol'].values) + list(portfolio['sold']['symbol'].values), list(portfolio['open'].index.values) + list(portfolio['sold']['coin'].values))), pages=4) # a bit precautionary to have portfolio['sold'] values since these coins should be sold completely (unless fractions remain due to distribution) or if some remain due to incomplete order (and it would show up as balance_locked)
            ...
            print(str(assets) + "\nTotal Current Value: " + str(assets['current_value'].sum()) + "\nTotal Current Value (BTC): " + str(assets['current_value(btc)'].sum()) + "\nExecution time: " + str(time.time() - start_time) + "\n") # If open orders partially filled (and not filled - I think) balance_locked doesn't show as of 07/27/2020, can be misleading
            ...
            if binance_open_orders: # maybe refactor and looked at assets balance_locked, see if full balance is gone, if any locked make partial
                ...
                portfolio = retry_binance_open_orders_in_portfolio(portfolio=portfolio, binance_open_orders=binance_open_orders)
