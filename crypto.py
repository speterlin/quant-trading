...
def binance_trade_coin_btc(symbol_pair, trade=None, side=None, btc_invest=None, quantity=None, open_time=0.5, paper_trading=True, other_notes=None): # don't like non-boolean value for trade value but having two bool values would complicate matters (i.e if both set True etc.) # price_in_btc=None # assuming binance processes open_order (if order not immediately filled) within half second (and not enough time for open_order to execute, avoid ~Filled), might be able to use recvWindow with api call
    ...
    if not order['fills']: # maybe refactor might be able to use 'executedQty'
        time.sleep(open_time) # since order not often filled immediately (and then open_order not often created immediately), and don't want to call open_orders twice
                ...
                if symbol_pair in binance_pairs_with_price_current: # not retrieving new prices since whole function should execute (if done over one DAYS period) quickly
                    ...
                    if back_testing: # maybe refactor and put this logic into function binance_trade_coin_btc, maybe also include binance_check_24h_vol_and_price_in_btc in back_testing purchases
                        ...
                        if binance_btc_24h_vol_in_btc_too_low or binance_price_in_btc_mismatch:
                            continue
                        quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, trade="buy", btc_invest=btc_invest, open_time=0.5, paper_trading=paper_trading) # maybe refactor and change binance_coin_btc... to binance_btc_..., binance_coin_btc may be good to reflect importance (exchange order)
    ...
    for coin, market_cap_rank_change in coins_to_sell:
        if coin in portfolio['open'].index: # can add short logic # not accounting for if there is a market data issue (MDI when backtesting) - if sell price and roi doesn't reflect actual, can try to postpone selling by a day # (market_cap_rank_change <= DOWN_MOVE) and
            ...
            if back_testing:
                ...
                sell_date, binance_btc_24h_vol_in_btc = datetime.now(), float(_fetch_data(binance_client.get_ticker, params={'symbol': symbol_pair}, error_str=" - Binance get ticker error for symbol_pair: " + symbol_pair + " on date: " + str(datetime.now().date()), empty_data={})['quoteVolume']) # *binance_pairs_with_price_current['BTCUSDT'] # refactor - complicates matter when have both buy_date and rr_sell sell_date binance_btc_24h_vol_in_btc, keep if want to check for pump and dump
                ...
                quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, trade="sell", quantity=balance, open_time=0.5, paper_trading=(True if portfolio['open'].loc[coin, 'position'] == 'long-p' else paper_trading))
                sell_price_in_btc, other_notes = price_in_btc, None
                roi_in_btc = (sell_price_in_btc - portfolio['open'].loc[coin, 'buy_price(btc)']) / portfolio['open'].loc[coin, 'buy_price(btc)'] # maybe refactor and make buy_price_in_btc it's own variable in previous line
...
# assets 'FET', 'KMD' leftover from cryptohopper trades (unable to trade such a small amount), 'TFUEL' because THETA funds in your account on May 2020, Unsure about 'TOMO' but small amount and not always in top 250 so needed since sometimes: 504 Server Error: Gateway Time-out, 'GTO' and 'BTG' because not part of portfolio (part of arbitrage) keep getting KeyError: 'btg'/'gto' when have 504 Server Error: Gateway Time-out
            ...
            if price_in_btc: # maybe refactor check for if 'position' == 'long'/'long-p' in case implement shorting, price to ensure price was calculated on coingecko
                ...
                if tsl_armed:
                    ...
                    if tsl_price_in_btc_change <= TRAILING_STOP_LOSS_PERCENTAGE: # should check if price on coingecko is equal/close to price in binance
                        ...
                        quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, trade="sell", quantity=balance, open_time=0.5, paper_trading=(True if portfolio['open'].loc[coin, 'position'] == 'long-p' else paper_trading), other_notes=other_notes + " at roi " + str(price_in_btc_change))
                    ...
                    if not df_matching_open_options.empty: # maybe refactor and add precautionary check - shouldn't have to check len() == 1 since symbol, like coin (index) should be unique in portfolio['open'] unless two different coins have the same symbol
                        ...
                        if abs((binance_pairs_with_price_current[symbol_pair] - portfolio['open'].loc[coin, 'buy_price(btc)']) / portfolio['open'].loc[coin, 'buy_price(btc)']) <= OPEN_ORDER_PRICE_DIFFERENCE: # *binance_pairs_with_price_current['BTCUSDT'] # refactor can make that a parameter also not sure if necessary, can be sl/2
                            ...
                            if resp['status'] == 'CANCELED': # cancelled is more British English, canceled is more American English, Binance uses canceled, grammarly.com/blog/canceled-vs-cancelled
                                ...
                                quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, side=side, quantity=balance, open_time=0.5, paper_trading=paper_trading, other_notes="Retrying open order for coin " + coin + " bought on " + str(order_time))
                    ...
                    if not df_matching_sold_options.empty: # maybe refactor and add precautionary check - shouldn't have to check len() == 1 since symbol, trade_notes and order_time time frame (+/- 10 minutes)
                        ...
                        if abs((binance_pairs_with_price_current[symbol_pair] - portfolio['sold'].loc[idx, 'sell_price(btc)']) / portfolio['sold'].loc[idx, 'sell_price(btc)']) <= OPEN_ORDER_PRICE_DIFFERENCE: # *binance_pairs_with_price_current['BTCUSDT'] # refactor can make that a parameter also not sure if necessary, can be sl/2
                            ...
                            if resp['status'] == 'CANCELED': # cancelled is more British English, canceled is more American English, Binance uses canceled
                                ...
                                quantity, price_in_btc, binance_coin_btc_order, binance_coin_btc_open_orders, trade_notes = binance_trade_coin_btc(symbol_pair=symbol_pair, side=side, quantity=balance, open_time=0.5, paper_trading=paper_trading, other_notes="Retrying open order for coin " + portfolio['sold'].loc[idx, 'coin'] + " sold on " + str(order_time))
                                new_price_in_btc = (price_in_btc*quantity + original_price_in_btc*executed_quantity) / original_quantity
